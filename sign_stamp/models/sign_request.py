...
from odoo.tools.pdf import PdfFileReader, PdfFileWriter, reshape_text
...
class SignRequest(models.Model):
    ...
    def _generate_completed_document(self, password=""):
                ...
                for item in items:
                    ...
                    if frame:
                        try:
                            image_reader = ImageReader(
                                io.BytesIO(
                                    base64.b64decode(frame[frame.find(",") + 1 :])
                                )
                            )
                    ...
                    if item.type_id.item_type == "text":
                        ...
                        if item.alignment == "left":
                            can.drawString(
                                width * item.posX,
                                height * (1 - item.posY - item.height * 0.9),
                                value,
                            )
                        ...
                        string_width = stringWidth(
                            text.replace("<strike>", "").replace("</strike>", ""),
                            font,
                            font_size,
                        )
                        p = Paragraph(
                        ...
                        posX = (
                            width * (item.posX + item.width * 0.5) - string_width // 2
                        )
                        posY = (
                            height * (1 - item.posY - item.height * 0.5)
                            - p.wrap(width, height)[1] // 2
                        )
                        ...
                        y = 1 - item.posY
                        for line in lines:
                            empty_space = width * item.width - can.stringWidth(
                                line, font, font_size
                            )
                            x_shift = 0
                            ...
                            can.drawString(
                                width * item.posX + x_shift, height * y, line
                            )
                        ...
                        item.type_id.item_type == "signature"
                        or item.type_id.item_type == "initial"
                        or item.type_id.item_type == "stamp"
                    ):
                        try:
                            image_reader = ImageReader(
                                io.BytesIO(
        ...
        attachment_log = self.env["ir.attachment"].create(
            {
                "name": "Certificate of completion - %s.pdf"
        ...
        self.completed_document_attachment_ids = [
            Command.set([attachment.id, attachment_log.id])
        ]
...
class SignRequestItem(models.Model):
    ...
    def _get_user_signature(self, signature_type="sign_signature"):
        ...
        if sign_user and signature_type in [
            "sign_signature",
            "sign_initials",
            "sign_stamp",
        ]:
