...
// Patching SignablePDFIframe.prototype to add custom behavior
patch(SignablePDFIframe.prototype, {
    enableCustom(signItem) {
        // Call the parent method
        super.enableCustom(signItem);

        // Get the element, data, and type of the sign item
        ...
        // Check if the item is of type 'stamp'
    ...
    openSignatureDialog(signatureItem, type) {
        ...
        // get all data required for dialog
        const dataRequired = this.dataRequiredForDialog(signatureItem, type);
    ...
    dataRequiredForDialog(signatureItem, type){
        const signature = {
    ...
    // RPC to save user stamp/signature/initials data
    updateUserSignature(type) {
        return rpc("/sign/update_user_signature", {
            ...
            signature_type: type.item_type === "signature" ? "sign_signature" :
                            type.item_type === "stamp" ? "sign_stamp" : "sign_initials",
    ...
    getSignatureValueFromElement(item){
        if(item.data.type === "stamp"){
            // Get base64 image string from dataset if item is a stamp
