...
class EstateProperty(models.Model):
    ...
    def _check_selling_price(self):
        for record in self:
            if record.selling_price != 0 and float_compare(record.selling_price, record.expected_price * 0.9, precision_rounding=0.01) < 0:
    ...
    def _check_state(self):
        for record in self:
            if record.state not in ("new", "canceled"):
                raise UserError(self.env._("Cannot delete property if state isn't new or canceled"))
    ...
    def action_sold(self):
        for record in self:
            if record.state == "canceled":
                raise UserError(self.env._("Cannot set property to sold if it is canceled"))
            record.state = "sold"
        return True
    ...
    def action_cancel(self):
        for record in self:
            if record.state == "sold":
                raise UserError(self.env._("Cannot set property to canceled if it is sold"))
