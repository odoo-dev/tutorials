...
publicWidget.registry.FullCalendarWidget = publicWidget.Widget.extend({
    ...
    start: async function () {
        const loggedInUser = JSON.parse($(".logged_in_user").attr("value") || "[]");
        let slotsData = JSON.parse($(".events_fullcalendar_data").attr("value") || "[]");
        try {
            const userTimeZone = Intl.DateTimeFormat().resolvedOptions().timeZone;

            let calendarHeaders = {
                left: 'dayGridMonth,timeGridWeek,listMonth,dayGridDay',
                center: 'title',
            ...
            this.calendar = new FullCalendar.Calendar(document.querySelector("#calendar"), {
                initialView: slotsData.length === 0 ? 'listMonth' : 'dayGridMonth',
                events: slotsData,
                noEventsContent: { html: this.noEventsContent(loggedInUser) },
    ...
    onEventDidMount: function (calRender) {
        const eventContent = calRender.el.querySelectorAll('.fc-event-title');
        ...
        if (calRender.event.extendedProps.isCancelled) calRender.el.classList.add("text-decoration-line-through")
        calRender.el.classList.add('cursor-pointer');
        calRender.el.childNodes[0].classList.add('fw-bold');
    },
    noEventsContent: function (user) {
        return markup(`
                <div class="col-12 text-center">
                    ...
                    <h3>Hey ${user.name}, no lectures yet? 🤔</h3>
                    <p>Enjoy the calm before the storm! Or maybe sign up for something before boredom wins. 😆📚</p>
                </div>
    `)
    },
    ...
    onEventClick: function (info) {
        ...
        if (info.event.extendedProps.isCancelled) {
            attendedBadge.text(_t("Lecture Cancelled")).removeClass("bg-danger", "bg-success").addClass("bg-warning");
        } else if (info.event.extendedProps.attended) {
